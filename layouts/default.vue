		...
		<header class="no-print sticky top-0 z-40 border-b border-black/10 bg-[var(--ui-primary)] text-white backdrop-blur">
			<div class="flex w-full items-center justify-between gap-1.5 px-2 py-1.5 md:gap-3 md:py-2 md:px-3">
				<button class="flex min-w-0 cursor-pointer items-center gap-1.5 leading-none md:gap-2" @click="navigateTo({ name: 'Canvas' })">
					...
					<span class="text-base tracking-tight md:text-lg">
				...
				<div v-if="!songStore.user" class="flex shrink-0 items-center gap-1.5 md:gap-2">
					<UTooltip text="Ir a inicio de sesión">
						<UButton size="xs" color="primary" variant="soft" class="whitespace-nowrap border-transparent bg-white/10 text-white hover:bg-white/20 md:text-sm" :to="{ name: 'LoginPage' }">
							Entrar
						</UButton>
					</UTooltip>
					<UTooltip text="Ir a registro">
						<UButton size="xs" color="primary" variant="soft" class="whitespace-nowrap border-transparent bg-white/10 text-white hover:bg-white/20 md:text-sm" :to="{ name: 'RegisterPage' }">
							Registro
						</UButton>
					</UTooltip>
				</div>
				<div v-else class="flex shrink-0 items-center gap-1.5 md:gap-2">
					<UTooltip text="Ver listado de partituras">
						<UButton size="xs" color="primary" variant="soft" class="whitespace-nowrap border-transparent bg-white/10 text-white hover:bg-white/20 md:text-sm" :to="{ name: 'ListPartituresPage' }">Listado</UButton>
					</UTooltip>
					<UTooltip text="Gestionar páginas públicas">
						<UButton size="xs" color="primary" variant="soft" class="whitespace-nowrap border-transparent bg-white/10 text-white hover:bg-white/20 md:text-sm" :to="{ name: 'ListPublicPages' }">Páginas</UButton>
					</UTooltip>
					<UTooltip text="Cerrar sesión">
						<UButton size="xs" color="primary" variant="soft" class="whitespace-nowrap border-transparent bg-white/10 text-white hover:bg-white/20 md:text-sm" @click="logout">
							Salir
						</UButton>
					</UTooltip>
				</div>
